import { ArrowUpRight, CheckCircle2, FlaskConical, Radio } from "lucide-react";
...
const pipelineSteps = [
  ["Goal captured", "Increase signup conversion", "Input locked"],
  ["Experiment pipeline", "6 ranked growth tests", "AI sorted"],
  ["Variant ready", "Founder proof landing angle", "Ship next"],
] as const;
    ...
    >
      ...
      <div className="grid gap-3 motion-stagger">
        {pipelineSteps.map(([label, title, status], index) => (
          <div
            className="relative rounded-lg border border-white/10 bg-white/[0.045] p-3"
            key={label}
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="inline-flex size-8 items-center justify-center rounded-md border border-lime-300/20 bg-lime-300/10 text-lime-200">
                {index === 0 ? (
                  <Radio
                    aria-hidden="true"
                    className="size-4 motion-signal-pulse"
                  />
                ) : index === 1 ? (
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <MetricBeacon label="Ship queue" value="6" />
      </div>
    </PremiumSurface>
  );
}
